...
<form action="" method="POST">
    <table>
        <tr class="title">
            <th><a href="<c:url value="/${lnk}"><c:param name="sort.property" value="name"/></c:url>"><fmt:message
                    key="name"/></a></th>
            <th><a href="<c:url value="/${lnk}"><c:param name="sort.property" value="code"/></c:url>"><fmt:message
                    key="code"/></a></th>
        </tr>
        <tr class="title">
        ...
        <c:forEach items="${countries.pageList}" var="cntry" varStatus="s">
            <tr
                    <c:if test="${s.count % 2 == 0}">class="odd"</c:if>
        ...
        <c:if test="${countries.nrOfPages > 1}">
            ...
            <c:forEach begin="${countries.firstLinkedPage}" end="${countries.lastLinkedPage}" var="crtpg">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<c:url value="/${lnk}"><c:param name="page" value="${crtpg}"/></c:url>"><c:out
                                value="${crtpg + 1}"/></a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            &nbsp;...&nbsp;
            <a href="<c:url value="/${lnk}"><c:param name="page" value="${countries.nrOfPages - 1}"/></c:url>"><c:out
